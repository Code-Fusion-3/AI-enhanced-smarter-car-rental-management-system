...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <p class="text-3xl font-bold text-blue-600">$<?= number_format($totalCost ?? 0, 2) ?></p>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <p class="text-3xl font-bold text-blue-600">$<?= number_format($totalMaintenance > 0 ? ($totalCost / $totalMaintenance) : 0, 2) ?></p>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php foreach ($typeData as $type): ?>
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    $<?= number_format($type['total_cost'] ?? 0, 2) ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    $<?= number_format($type['average_cost'] ?? 0, 2) ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach ($carData as $car): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                $<?= number_format($car['total_cost'] ?? 0, 2) ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                $<?= number_format($car['maintenance_count'] > 0 ? ($car['total_cost'] / $car['maintenance_count']) : 0, 2) ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
...
<script>
    ...
    const maintenanceTypes = <?= json_encode(array_map(function($type) { return ucfirst($type['maintenance_type']); }, $typeData)) ?>;
    ...
    const carLabels = <?= json_encode(array_map(function($car) { return $car['make'] . ' ' . $car['model']; }, $carData)) ?>;
