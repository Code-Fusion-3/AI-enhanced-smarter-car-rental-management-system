    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
                <div class="border-b border-gray-200">
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        <button onclick="showTab('upcoming')" class="tab-button text-blue-600 border-blue-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                            Upcoming
                        ...
                        <button onclick="showTab('active')" class="tab-button text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 border-transparent font-medium text-sm">
                            Active
                        ...
                        <button onclick="showTab('past')" class="tab-button text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 border-transparent font-medium text-sm">
                            Past Rentals
                        </button>
                        <button onclick="showTab('all')" class="tab-button text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 border-transparent font-medium text-sm">
                            All Rentals
                        </button>
                    </nav>
                </div>
        ...
        <?php if (empty($rentals)): ?>
            <div class="text-center py-12 bg-white rounded-lg shadow">
                ...
                <div class="mt-6">
                    <a href="index.php?page=cars" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        <?php foreach ($upcomingRentals as $rental): ?>
                            <div class="bg-white overflow-hidden shadow rounded-lg">
                                <div class="relative">
                                    <img class="h-48 w-full object-cover" src="<?= $rental['image_url'] ?? 'assets/images/car1.png' ?>" alt="<?= $rental['make'] ?> <?= $rental['model'] ?>">
                                ...
                                <div class="p-5">
                                    <h3 class="text-lg font-medium text-gray-900"><?= $rental['make'] ?> <?= $rental['model'] ?> <?= $rental['year'] ?></h3>
                                    ...
                                    <div class="mt-4 flex justify-between">
                                        <div>
                                            ...
                                            <p class="text-lg font-semibold text-gray-900">$<?= number_format($rental['total_cost'], 2) ?></p>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        <?php foreach ($activeRentals as $rental): ?>
                            <div class="bg-white overflow-hidden shadow rounded-lg border-l-4 border-green-500">
                                <div class="relative">
                                    <img class="h-48 w-full object-cover" src="<?= $rental['image_url'] ?? 'assets/images/car.png' ?>" alt="<?= $rental['make'] ?> <?= $rental['model'] ?>">
                                    <div class="absolute top-0 right-0 mt-2 mr-2">
                                ...
                                <div class="p-5">
                                    ...
                                    <div class="mt-2 flex items-center">
                                        <svg class="flex-shrink-0 mr-1.5 h-5 w-5 <?= $isOverdue ? 'text-red-500' : 'text-yellow-500' ?>" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                ...
                <?php else: ?>
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        <?php foreach ($pastRentals as $rental): ?>
                            <div class="bg-white overflow-hidden shadow rounded-lg <?= $rental['status'] === 'completed' ? 'border-l-4 border-gray-300' : 'border-l-4 border-red-300' ?>">
                                <div class="relative">
                                    <img class="h-48 w-full object-cover" src="<?= $rental['image_url'] ?? 'assets/images/car.png' ?>" alt="<?= $rental['make'] ?> <?= $rental['model'] ?>">
                                    <div class="absolute top-0 right-0 mt-2 mr-2">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?= $rental['status'] === 'completed' ? 'bg-gray-100 text-gray-800' : 'bg-red-100 text-red-800' ?>">
                                            <?= ucfirst($rental['status']) ?>
                                        </span>
                                    </div>
                                </div>
                    ...
                    <div class="bg-white shadow overflow-hidden sm:rounded-md">
                        <ul class="divide-y divide-gray-200">
                            <?php foreach ($rentals as $rental): ?>
                                <li>
                                    <a href="index.php?page=rentals&action=view&id=<?= $rental['rental_id'] ?>" class="block hover:bg-gray-50">
                                        <div class="flex items-center px-4 py-4 sm:px-6">
                                            <div class="min-w-0 flex-1 flex items-center">
                                                <div class="flex-shrink-0 h-16 w-16 bg-gray-100 rounded-md overflow-hidden">
                                                    <img src="<?= $rental['image_url'] ?? 'assets/images/car.png' ?>" alt="<?= $rental['make'] ?> <?= $rental['model'] ?>" class="h-full w-full object-cover">
                                                </div>
                                            ...
                                            <div class="ml-5 flex-shrink-0 flex flex-col items-end">
                                                ...
                                                <p class="mt-2 text-sm font-medium text-gray-900">$<?= number_format($rental['total_cost'], 2) ?></p>
                                            </div>
                                            <div class="ml-5">
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <form id="review-form" action="index.php?page=rentals&action=submit_review" method="POST" class="mt-5">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex space-x-2">
                            <div class="rating-stars flex">
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                <button type="button" data-rating="<?= $i ?>" class="rating-star p-1 focus:outline-none">
                                    <svg class="h-8 w-8 text-gray-300 hover:text-yellow-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                    </svg>
                                </button>
                                <?php endfor; ?>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="feedback" name="feedback" rows="4" class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Share your experience with this rental..."></textarea>
                    </div>
                </form>
            </div>
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button type="button" onclick="submitReview()" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">
                    Submit Review
                </button>
