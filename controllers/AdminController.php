        ...
        switch ($subaction) {
            case 'add':
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    if ($question && $answer) {
                        $stmt = $this->db->prepare('INSERT INTO faqs (question, answer, keywords, active) VALUES (?, ?, ?, ?)');
                        $stmt->bind_param('sssi', $question, $answer, $keywords, $active);
                        $stmt->execute();
                        $_SESSION['success'] = 'FAQ added successfully.';
                        header('Location: index.php?page=admin&action=admin_faqs');
                        exit();
                    } else {
                        $_SESSION['error'] = 'Question and answer are required.';
                    }
                }
                require 'views/admin/faqs/form.php';
                break;
            case 'edit':
                $faq = null;
                if ($faqId > 0) {
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    $active = isset($_POST['active']) ? (int) $_POST['active'] : 1;
                    if ($question && $answer) {
                        $stmt = $this->db->prepare('UPDATE faqs SET question=?, answer=?, keywords=?, active=? WHERE id=?');
                        $stmt->bind_param('sssii', $question, $answer, $keywords, $active, $faqId);
                        $stmt->execute();
            ...
            case 'toggle':
                if ($faqId > 0) {
                    $stmt = $this->db->prepare('UPDATE faqs SET active = NOT active WHERE id = ?');
                    $stmt->bind_param('i', $faqId);
                    $stmt->execute();
            ...
            default:
                $faqs = [];
                $result = $this->db->query('SELECT * FROM faqs ORDER BY id DESC');
                while ($row = $result->fetch_assoc()) {
                    $faqs[] = $row;
                }
                require 'views/admin/faqs/list.php';
        }
    }
}